...
describe('real-Tally structures (pilot calibration, 2026-08-20)', () => {
  ...
  it('takes the side from the amount sign, so a negative debit posts as a credit (Garima March 2025 TDS leg)', () => {
    // Purchase keyed with the TDS leg on the Dr side as a negative figure:
    // ISDEEMEDPOSITIVE=Yes but AMOUNT positive. Tally posts it as Cr 5,000
    // (the voucher only balances that way: 54,000 + 5,000 = 59,000).
    const xml = `<ENVELOPE><BODY><IMPORTDATA><REQUESTDATA><TALLYMESSAGE><VOUCHER VCHTYPE="Purchase"><DATE>20250305</DATE><NARRATION>x</NARRATION><VOUCHERTYPENAME>Purchase</VOUCHERTYPENAME><ALLLEDGERENTRIES.LIST><LEDGERNAME>Sharma Legal</LEDGERNAME><ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE><AMOUNT>54000.00</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>Professional Fees</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>-50000.00</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>CGST</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>-4500.00</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>SGST</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>-4500.00</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>TDS Payable u/s 194J</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>5000.00</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>Zero Leg</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>0.00</AMOUNT></ALLLEDGERENTRIES.LIST></VOUCHER></TALLYMESSAGE></REQUESTDATA></IMPORTDATA></BODY></ENVELOPE>`;
    const legs = parseDayBookXml(utf16(xml)).vouchers[0].ledgerEntries;
    expect(legs.map((l) => `${l.drOrCr} ${l.ledgerName} ${l.amount}`)).toEqual([
      'Cr Sharma Legal 54000',
      'Dr Professional Fees 50000',
      'Dr CGST 4500',
      'Dr SGST 4500',
      'Cr TDS Payable u/s 194J 5000',
      'Dr Zero Leg 0',
    ]);
    const dr = legs.filter((l) => l.drOrCr === 'Dr').reduce((s, l) => s + l.amount, 0);
    const cr = legs.filter((l) => l.drOrCr === 'Cr').reduce((s, l) => s + l.amount, 0);
    expect(dr).toBe(cr);
  });
