...
describe('head-wise GST payable ledgers fill the key\'s GST Payable leg', () => {
  const tags: AnswerKeyEntry['concept_tags'] = ['journal_voucher_basics'];
  const key: AnswerKey = {
    entries: [
      leg(1, 'Output CGST', 'Dr', 100, 'Journal', { gst_head: 'CGST', concept_tags: tags }),
      leg(1, 'Output SGST', 'Dr', 100, 'Journal', { gst_head: 'SGST', concept_tags: tags }),
      leg(1, 'Output IGST', 'Dr', 300, 'Journal', { gst_head: 'IGST', concept_tags: tags }),
      leg(1, 'GST Payable', 'Cr', 500, 'Journal', { concept_tags: tags }),
    ],
  };
  const journal = (payables: [string, number][]) =>
    voucher('Journal', [
      entry('Output CGST', 'Dr', 100),
      entry('Output SGST', 'Dr', 100),
      entry('Output IGST', 'Dr', 300),
      ...payables.map(([name, amount]) => entry(name, 'Cr', amount)),
    ]);
  const accounts = (payables: [string, number][]) => diffOf(score([journal(payables)], key), 1, 'account');

  it('accepts one head-wise payable and a split across head-wise payables', () => {
    expect(accounts([['GST Payable', 500]]).every((diff) => diff.is_correct)).toBe(true);
    expect(accounts([['IGST Payable', 500]]).every((diff) => diff.is_correct)).toBe(true);
    expect(accounts([['CGST Payable', 100], ['SGST Payable', 100], ['IGST Payable', 300]]).every((diff) => diff.is_correct)).toBe(true);
  });

  it('keeps rejecting an Output ledger credited as the payable', () => {
    const v = voucher('Journal', [entry('Output CGST', 'Dr', 100), entry('Output SGST', 'Dr', 100), entry('Output IGST', 'Dr', 300), entry('Output IGST', 'Cr', 500)]);
    expect(diffOf(score([v], key), 1, 'account')[3].error_code).toBe('ACCOUNT_WRONG');
  });
});
...
describe('payment of last month\'s GST is judged against the learner\'s own payable', () => {
  const key: AnswerKey = {
    entries: [
      leg(1, 'GST Payable', 'Dr', 50000, 'Payment', { concept_tags: ['gst_payment'] }),
      leg(1, 'HDFC Bank — 1234', 'Cr', 50000, 'Payment', { concept_tags: ['gst_payment'] }),
    ],
  };
  const pay = (amount: number) => voucher('Payment', [entry('GST Payable', 'Dr', amount), entry('HDFC Bank — 1234', 'Cr', amount)]);
  const withOpening = (payable: number): ParsedTrialBalance => ({
    ledgers: [{ ledgerName: 'GST Payable', openingDebit: 0, openingCredit: payable, closingDebit: 0, closingCredit: 0 }],
  });
  const amounts = (paid: number, tb: ParsedTrialBalance) => scoreSubmission({ vouchers: [pay(paid)] }, tb, key).per_voucher_diffs.filter((d) => d.field === 'amount');

  it('accepts the key figure, or the payable the learner\'s own Trial Balance opens with', () => {
    expect(amounts(50000, EMPTY_TB).every((d) => d.is_correct)).toBe(true);
    expect(amounts(48000, withOpening(48000)).every((d) => d.is_correct)).toBe(true);
  });

  it('flags a payment that matches neither', () => {
    expect(amounts(45000, withOpening(48000)).some((d) => d.error_code === 'AMOUNT_WRONG')).toBe(true);
  });
});
