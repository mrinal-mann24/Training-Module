...
describe('insertConceptAttempts', () => {
  it('upserts with ignoreDuplicates on (learner_id, exercise_id, concept_tag) instead of a plain insert', async () => {
    const supabase = fakeSupabase();
    const attempts = [{ conceptTag: 'gst_classification' as ConceptTag, result: 'pass' as const, hintRungsUsed: 0 }];

    await insertConceptAttempts(supabase, 'learner-1', 'exercise-1', attempts);
