...
`.night-grain`, `.night-scrim`, `.night-page` (the three-row
...
- **VideoBackdrop** (`app/components/VideoBackdrop.tsx`): the ambient loop plus its `.night-scrim`, which always ships with it. The clip is a bright, high-key illustration; the scrim grades from a light wash at the top (where the artwork should still read) down to solid black behind the hero copy and stats. Without it, white type on the video is unreadable — the Vesper reference's "100% opacity, no overlay" assumes a dark clip, which this is not.
