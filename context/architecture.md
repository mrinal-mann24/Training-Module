  ...
  /tutor                      → Grading/generation engine, grouped by concern:
    generate-exercise.ts, generate-review-exercise.ts, assign-pack-exercise.ts, select-exercise-kind.ts,
      select-batch-concepts.ts, generation-checks.ts, documents-mode.ts
                              → Pack assignment, the LLM-generated diagnostic, and the generation checks shared with the engine below
    generate-planned-exercise.ts, build-key/, key-invariants.ts, party-master.ts, ledger-state.ts, bill-reference.ts, carried-rectifications.ts
...
6. **A generated exercise's answer key is immutable once created.** The same answer key that scored the first submission for that exercise scores any resubmission for it. Regenerating or editing an answer key after the fact would silently invalidate prior scoring and break the mastery history's integrity. A defect that was ours in a delivered batch is never fixed by rewriting the key either (decided 2026-09-22 after the historical audit): the learner's books mirror the key they were scored against, so a rewritten key would make every later comparison of their books report false differences. The owner records the correcting journal in `carried_rectifications` (service-role only) and the learner's next generated batch carries it as a real transaction with code-built text and legs (`lib/tutor/carried-rectifications.ts`), the way a firm rectifies a posted month; the row is stamped with the exercise that carried it. Scores already recorded are never recomputed.

7. **Answer keys are computed, never authored** (rebuild, 2026-09-22). On the planned engine (`learner_profile.generation_engine = 'planned'`) the LLM returns only commercial events (`lib/schemas/batch-plan.ts`); every ledger leg, GST head and amount, TDS figure, bill allocation, concept tag, opening balance and learner-facing line is built by code from `LedgerState` (`lib/tutor/ledger-state.ts`) and the party master (`lib/tutor/party-master.ts`) in `lib/tutor/build-key/`, and `assertKeyValid` (`lib/tutor/key-invariants.ts`) runs on the exact object that is persisted. A plan the books cannot support is regenerated, never patched. Bill references are produced only by `formatBillReference` (`lib/tutor/bill-reference.ts`). Reverse charge is likewise never the plan's choice: the builder applies the rulebook category to every purchase and books the RCM journal itself; a customer's TDS on a receipt is computed on the taxable value the books remember for the invoice. Since Stage 6 the builder covers every concept tag, and the legacy LLM-authored generator (`generateAdaptiveExercise`, its prompt and `advance-adjustment.ts`) was deleted on 2026-09-22 at the owner's instruction: the planned engine is the only generator. The checks the legacy generator shared (`runGenerationChecks`, `finalizeBatch`, `planSourceDocuments`, `prepareSourceDocuments`) stay in `generate-exercise.ts`, used by the final invariant and by the LLM-generated diagnostic for learners without a pack. The model answers in a flat wire shape (`lib/llm/prompts/batch-plan-json-schema.ts`: one event object, nullable fields, no `anyOf`, no enum on a nullable type, because the provider's strict JSON mode refuses both) which `batchPlanFromWire` (`lib/schemas/batch-plan-wire.ts`) converts to the typed plan with nothing guessed. Every free-text field of the plan is validated before it can reach a key or a learner (`lib/tutor/build-key/validate.ts`: party names, ledgers, document numbers, line text, scenario), all figures are whole rupees, and a plan that fails is sent back, never repaired. When a planned batch cannot be built after three plans, generation throws, the job step retries with a fresh plan, and `scripts/advance-learner.ts` regenerates by hand if every retry fails; there is no second generator to fall back to. `learner_profile.generation_engine` is no longer read. The saved key records which engine built it (`answer_key.engine`) and which carried rectifications it contains (`answer_key.carried_rectification_ids`). `scripts/dry-run-planned-batch.ts` runs the real pipeline for a learner against their real books and saves nothing; run it before switching a learner or changing the schema, prompt or builder.
...
- **Mastery counts one attempt per concept per exercise (2026-09-17).** `collapseCorrectionRounds` in `lib/tutor/mastery.ts` keeps each exercise's latest round, so correction rounds can never master or escalate a concept on their own. A pass reached only after a correction round is not a clean pass: it resets the streak, and it does not count as a failure for escalation or reinforcement.
