...
10. **Feedback.** The tutor responds: one plain line orienting the learner on the batch, specific honest praise for what was done well, areas flagged to re-look at (not the exact error, on first pass), a note on whether any previously failed concept is now fixed, still failing, or newly broken, and one line on what's next. **No score, no percentage and no pass/fail verdict ever reaches the learner** (2026-09-16): the weighted score and the pass/partial/fail result still exist server-side and still drive mastery, but what the learner reads is what they understood and what they are still working on.
11. **Help ladder, and the correction loop.** The help ladder is 3 steps: a reference-video pointer with a guiding question, an indirect hint naming what to rework, and finally the full worked answer with a check-for-understanding question. The learner can ask for help at any time, and from 2026-09-16 the tutor also **pushes** the next step without being asked: a batch with a concept wrong does not move the learner on. It gives the step aimed at that concept and asks for corrected exports, the learner fixes it in Tally and re-uploads, and the corrected submission is scored against the same answer key. Three rounds, one per step; after the third the learner has the worked answer, so the loop closes either way and nobody is left permanently stuck. The concept still counts as weak and comes back later in a different question.
...
- **Smart Send (2026-09-15)**: on explain/review exercises, one Send button. Only a clear question (a trailing "?", or an opener like what/how/which) is answered immediately. Everything else, including short or informal explanations, is shown for confirmation: "Send this as your explanation?" with Submit answer / It's a question, so the learner decides. Only a tapped "Submit answer" scores and locks the exercise. (2026-09-16: an AI tie-break used to decide unclear text and answered short explanations as questions, which left them impossible to file. It was removed; a wrong confirmation costs one tap, a wrong question verdict cost the explanation.)
- Graduated 3-step help ladder with anti-gaming (probes for explanation before crediting mastery on suspiciously clean answers), pushed automatically on a failing concept as well as available on request
...
- Progress view showing the five learner-facing modules (Sales and Receivables, Purchases and Payables, Banking, GST and TDS, Month End and Assets) and flagged escalation concepts, plus a single overall progress bar on the dashboard. The detailed per-concept module structure stays in the backend and still gates advancement
...
- Full coaching engine: help ladder, the correction loop, Socratic feedback, two-lane tone, anti-gaming
...
8. The learner completes the capstone, sees a diagnostic re-run showing measurable improvement over their original diagnostic, and receives a certificate. The improvement is shown as concepts that were failing and are now clean, never as a before-and-after percentage (2026-09-16).
