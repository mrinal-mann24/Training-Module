...
//
//   npx tsx scripts/advance-learner.ts --learner praveen.naidu@AI Accountant'scard.com
...
async function main(): Promise<void> {
  ...
  if (!email) {
    throw new Error(
      "Usage: npx tsx scripts/advance-learner.ts --learner <email> [--replace-latest]",
    );
  }
  ...
  if (profileError || !profile)
    throw profileError ?? new Error("learner_profile missing");
  ...
  if (!lastScored)
    throw new Error("No scored submission — nothing to advance from.");
  ...
  const { data: scoredExercise, error: scoredError } = await supabase
    ...
    .single();
  if (scoredError || !scoredExercise)
    throw scoredError ?? new Error("scored exercise missing");
  // difficulty_level lives inside the scenario JSON, not as a column.
  const scoredScenario = scoredExercise.scenario as {
    difficulty_level?: string;
  };
  const previousDifficultyLevel = (scoredScenario.difficulty_level ??
    "L1") as Parameters<
    typeof generateNextExercise
  >[1]["previousDifficultyLevel"];
  ...
  console.log(
    `last scored: ${scoredExercise.kind} (${previousDifficultyLevel}) submitted ${lastScored.created_at}`,
  );
  console.log(
    `newest exercise: ${latest ? `${latest.kind} created ${latest.created_at}` : "(none)"}`,
  );
  ...
  if (
    replaceLatest &&
    latest &&
    latest.created_at > scoredExercise.created_at
  ) {
    ...
    if ((count ?? 0) > 0) {
      throw new Error(
        `Newest exercise ${latest.id} already has ${count} submission(s); refusing to delete it.`,
      );
    }
  ...
  if (created) {
    ...
    console.log(
      `newest now: ${created.kind} ${created.id} created ${created.created_at}, ${scenario.transactions?.length ?? 0} transactions`,
    );
  }
}
